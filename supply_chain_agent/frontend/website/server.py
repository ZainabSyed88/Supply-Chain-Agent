...
def start_server():
    ...
    # Create a custom TCPServer that allows socket reuse
    ...
    try:
        ...
        with ReusingTCPServer(("0.0.0.0", PORT), Handler) as httpd:
            local_ip = get_local_ip()
            
            print("[*] ChainPulse Website Server Started!")
            print(f"\n[OK] Open your browser to:\n")
        ...
        if e.errno == 48 or e.errno == 98 or e.errno == 10048:  # Port already in use
            print(f"\n[ERROR] Port {PORT} is already in use!")
            print(f"[*] Try: http://localhost:{PORT}")
            print("[*] Or kill existing process: taskkill /F /IM python.exe")
        else:
            print(f"\n[ERROR] Error: {e}")
        return False
