...
export default function AlertsFeed({ alerts = [], onAskAi }) {
  ...
  return (
    <div className="space-y-3">
      {alerts.map((alert) => (
        <div key={alert.id} className="rounded-lg border bg-white p-4 shadow-card">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2">
                <Badge label={statusLabel(alert.severity || "info")} variant={alert.severity || "info"} />
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-50"
            >
