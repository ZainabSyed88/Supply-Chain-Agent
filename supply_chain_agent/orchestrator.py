...
class SupplyChainOrchestrator:
    ...
    def __init__(self):
        if not HAS_OPENAI:
            raise RuntimeError("OpenAI package not installed. Install dependencies with `pip install -r requirements.txt`.")
        if not OPENAI_API_KEY:
            raise RuntimeError("OPENAI_API_KEY is not set. Add it to your .env file before running the orchestrator.")
    ...
    def execute_agent_3_risk_assessor(self) -> Dict:
        ...
        result = self.risk_assessor.execute(self.disruptions, "data/shipments.json")
    ...
    try:
        ...
        with open("output/pipeline_results.json", "w") as f:
            serializable_results = {
                ...
                for key, value in orchestrator.results.items()
            }
